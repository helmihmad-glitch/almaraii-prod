...
import { ArrowLeft, CalendarDays, Clock, Database, Pencil, Plus, Save, ShieldCheck, Trash2 } from "lucide-react";
...
export default function DailyProgramData() {
  ...
  const refreshPrograms = async () => {
    await Promise.all([utils.dailyProgram.list.invalidate(), utils.dailyProgram.byDate.invalidate()]);
  };
  ...
  const deleteProgram = trpc.dailyProgram.delete.useMutation({ onSuccess: async () => { await refreshPrograms(); setOperatorName(""); setLineDraft(emptyLine()); toast.success("Programme journalier supprimé"); }, onError: (error) => toast.error(error.message || "Impossible de supprimer le programme.") });
  ...
  useEffect(() => {
    setOperatorName(program?.operatorName ?? "");
  ...
  const submitProgram = (event: React.FormEvent) => {
    ...
    const payload = { programDate: selectedDate, operatorName: operatorName.trim(), actionPassword };
    if (!payload.operatorName) { toast.error("Indiquez le pupitreur du programme."); return; }
  ...
  const removeLine = (id: number) => {
    if (!requirePassword()) return;
    if (window.confirm("Supprimer cette ligne du programme ?")) deleteLine.mutate({ id, actionPassword });
  };
  const removeProgram = () => {
    if (!program || !requirePassword()) return;
    if (window.confirm(`Supprimer le programme du ${formatDate(selectedDate)} et toutes ses lignes ?`)) deleteProgram.mutate({ id: program.id, actionPassword });
  };

  return (
    <main className="daily-program-screen">
      <header className="daily-program-topbar">
        <Link href="/programme-journalier" className="daily-program-back"><ArrowLeft size={16} />Voir le programme</Link>
      ...
      <section className="daily-program-page">
        <div className="daily-program-hero daily-program-data-hero"><div><span className="daily-program-kicker"><Database size={14} />Administration</span><h1>Programme journalier <em>donnée</em></h1><p>Créez les programmes quotidiens, planifiez les articles, puis modifiez ou supprimez chaque ligne si nécessaire.</p></div><label className="daily-program-date"><span>Date à gérer</span><div><CalendarDays size={16} /><input type="date" value={selectedDate} onChange={(event) => setSelectedDate(event.target.value)} aria-label="Choisir la date à gérer" /></div></label></div>
