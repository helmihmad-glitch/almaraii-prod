...
export default function DailyProgramData() {
  ...
  const programsList = programsQuery.data ?? [];
  const {
    page: programsPage, setPage: setProgramsPage, pageCount: programsPageCount, pageItems: pagePrograms, pageSize: programsPageSize, setPageSize: setProgramsPageSize,
  } = usePagination(programsList, PROGRAMS_PAGE_SIZE);
  ...
  const submitProgram = (event: React.FormEvent) => {
    ...
    const payload = { programDate: selectedDate, operatorName: selectedOperatorNames.join(" · ") };
  ...
  const submitLine = (event: React.FormEvent) => {
    ...
    const payload = { programId: program.id, sequence: Number(lineDraft.sequence), article: lineDraft.article, version: lineDraft.version, bagQuantity: lineDraft.bagQuantity, bulkQuantity: lineDraft.bulkQuantity, plannedStart: lineDraft.plannedStart, plannedEnd: lineDraft.plannedEnd, observation: lineDraft.observation };
  ...
  const removeLine = (id: number) => { if (window.confirm("Supprimer cette ligne du programme ?")) deleteLine.mutate({ id }); };
  const removeProgram = () => { if (program && window.confirm(`Supprimer le programme du ${formatDate(selectedDate)} et toutes ses lignes ?`)) deleteProgram.mutate({ id: program.id }); };
          ...
          <div className="entry-overlay" role="dialog" aria-modal="true" aria-label="Confirmer l’import du classeur">
            <form className="entry-modal" onSubmit={submitImport}>
              ...
              <div className="entry-modal-actions">
                <button type="button" className="daily-program-secondary" onClick={() => setPendingImport(null)} disabled={isImporting}>Annuler</button>
