...
import { ArrowLeft, BookOpen, KeyRound, Plus, Settings2, ShieldCheck, Trash2 } from "lucide-react";
...
export default function Settings() {
  ...
  const addArticle = trpc.settings.addArticle.useMutation({
    onSuccess: async () => {
      await utils.settings.listArticles.invalidate();
      setArticleCode("");
  ...
  const archiveArticle = trpc.settings.archiveArticle.useMutation({
    onSuccess: async () => {
      await utils.settings.listArticles.invalidate();
      toast.success("Article retiré de la liste active");
    },
  ...
  const changePassword = trpc.settings.changeActionPassword.useMutation({
    onSuccess: () => {
      ...
      setArticlePassword("");
      toast.success("Mot de passe mis à jour", { description: "Les prochaines modifications et suppressions utiliseront ce nouveau mot de passe." });
    },
  ...
  const submitArticle = (event: React.FormEvent) => {
    ...
    if (!articlePassword) {
      toast.error("Saisissez le mot de passe de gestion pour modifier la liste.");
      return;
    }
    addArticle.mutate({ code: articleCode.trim().toUpperCase(), actionPassword: articlePassword });
  };

  const removeArticle = (id: number, code: string) => {
    if (!articlePassword) {
      toast.error("Saisissez le mot de passe de gestion avant de retirer un article.");
      return;
    }
    if (!window.confirm(`Retirer ${code} de la liste active ? L’historique de production restera conservé.`)) return;
    archiveArticle.mutate({ id, actionPassword: articlePassword });
  };
    ...
    <main className="settings-screen">
      ...
      <section className="settings-page">
        <div className="settings-hero">
          <div><span className="settings-kicker"><Settings2 size={14} />Administration</span><h1>Paramètres de <em>production</em></h1><p>Gérez les articles proposés à la saisie et le mot de passe exigé avant toute modification ou suppression du registre.</p></div>
        ...
        <div className="settings-grid">
          <article className="settings-card articles-card">
            ...
            <p className="settings-copy">Les articles ajoutés ici sont proposés dans le formulaire de production. Retirer un article ne modifie jamais les données déjà enregistrées.</p>
