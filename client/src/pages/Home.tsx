...
export default function Home() {
  ...
  const filteredDays = useMemo(() => registryDays.filter((day) => (articleFilter === "Toutes les lignes" || day.article === articleFilter) && (!query || day.article.toLowerCase().includes(query.toLowerCase()) || day.date.includes(query))), [registryDays, articleFilter, query]);
  const articles = useMemo(() => month.articles.length ? month.articles : Array.from(new Set(registryDays.map((d) => d.article))).map((article) => ({ article, production: registryDays.filter((d) => d.article === article).reduce((sum, d) => sum + d.production, 0), share: 0 })), [month, registryDays]);
