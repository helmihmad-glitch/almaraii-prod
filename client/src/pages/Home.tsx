...
export default function Home() {
  ...
  const verifyCommentPassword = trpc.production.verifyCommentPassword.useMutation();
  ...
  const openNewEntry = () => { setEditingId(null); setEditingSourceKey(null); setOriginalComment(""); setCommentPassword(""); setEntryForm({ ...emptyEntry, productionDate: new Date().toISOString().slice(0, 10) }); setIsEntryOpen(true); };
  const openEditEntry = (row: any) => { const comment = row.comment ?? ""; setEditingId(row.id ?? null); setEditingSourceKey(row.sourceKey ?? null); setOriginalComment(comment); setCommentPassword(""); setEntryForm({ productionDate: row.date, article: row.article, totalProductionHours: String(row.hours), plannedStopsHours: String(row.plannedStops), unplannedStopsHours: String(row.unplannedStops), productionTons: String(row.production), wasteTons: String(row.waste), standardRate: String(row.standardRate ?? 15), comment }); setIsEntryOpen(true); };
  ...
  const saveEntry = async (event: React.FormEvent) => {
    ...
    const commentChanged = comment !== originalComment;
    const needsCommentAuthorization = editingId ? commentChanged : Boolean(comment);
    if (needsCommentAuthorization) {
      if (!commentPassword) { toast.error("Saisissez le mot de passe pour enregistrer le commentaire."); return; }
      const verification = await verifyCommentPassword.mutateAsync({ password: commentPassword });
      if (!verification.authorized) { toast.error("Mot de passe de commentaire incorrect."); return; }
    }
    ...
    if (editingId) updateEntry.mutate({ id: editingId, ...payload, ...(commentChanged ? { comment, commentPassword } : {}) });
    ...
    else createEntry.mutate({ ...payload, ...(comment ? { comment, commentPassword } : {}) });
  };
